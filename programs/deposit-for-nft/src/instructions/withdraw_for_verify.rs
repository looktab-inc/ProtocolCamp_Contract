...
#[derive(Accounts)]
pub struct WithdrawForVerify<'info> {
...
pub fn handle(ctx: Context<WithdrawForVerify>, recommender_ratio: u8, client_ratio: u8) -> Result<()> {
    ...
    let recommender_amount = (DEPOSIT_PER_NFT as f64 * (0.01 * recommender_ratio as f64)) as u64;
    let client_amount = (DEPOSIT_PER_NFT as f64 * (0.01 * client_ratio as f64)) as u64;

    msg!("recommender_amount : {}", recommender_amount);
    ...
    if bank_account.nft_amount.remained() > 0 {
        ...
        system_program::transfer(cpi_to_bank_auth, recommender_amount)?;
